...
*/
function createGcode(n_points, initialheight, feedrate, ExtValueBuildUpPressure, ExtValueReleasePressure,pointshape,e){
  ...
  var p_counter=0;
  gcode=gcode+"G1 X"+pointshape[p_counter].x.toFixed(4)+" Y"+pointshape[p_counter].y.toFixed(4)+" Z"+initialheight+" F"+feedrate+"\n";
  ...
  var lastE= e[n_points]-ExtValueReleasePressure;
...
*/
function calculateExtrusionValues(numpoints,v_matdiameter,v_nozzlediameter,pointshape){
  var e = new Array;
  ...
  e[0]=0; /*first extrusion value*/
	...
	for (i = 1; i < numpoints; i = i+1) {
       var distance=Math.sqrt(Math.pow((pointshape[i].x-pointshape[i-1].x),2)+Math.pow((pointshape[i].y-pointshape[i-1].y),2));
       e[i]=e[i-1]+distance*nozzle_material_surfaces_ratio;   
	} 
  
	/*last extrusion value*/
		distance=Math.sqrt(Math.pow((pointshape[numpoints-1].x-pointshape[0].x),2)+Math.pow(((pointshape[numpoints-1].y-pointshape[0].y)),2));
...
function myprocess(processing) {
	...
	processing.draw = function() {							
		...
		var v_iheight = parseInt(document.getElementById("id_iheight").value);
		...
		/*initialiting variable n_selected */
		  if (optionselected == "Square") {var n_selected=1;}
		  if (optionselected == "Triangle") {var n_selected=2;}
		  if (optionselected == "Circle") {var n_selected=3;}
		...
		*/
		...
		switch(n_selected) {
									case 1:
										...
										e=calculateExtrusionValues(numpoints,v_matdiameter,v_nozzlediameter,pointshape); //calculating extruding values
										gcode=createGcode(numpoints,v_iheight,v_feedrate,v_ExtValueBuildUpPressure,v_ExtValueReleasePressure,pointshape,e); //create variable string gcode	
									break;
