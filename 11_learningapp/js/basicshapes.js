...
function myprocess(processing) {
	...
	processing.draw = function() {							
		...
		var v_npetalsflower = parseInt(document.getElementById("n_petals").value);
		
		var v_ncurvesroundedstar = parseInt(document.getElementById("n_curves").value);
		
		var v_sizeheart = parseInt(document.getElementById("n_sizeheart").value);
		...
		/*initialiting variable n_selected */
		  ...
		  if (optionselected == "Flower") {var n_selected=5;}	
		  if (optionselected == "Rounded Star") {var n_selected=6;}
		  if (optionselected == "Heart") {var n_selected=7;}
		...
		*/
		...
		switch(n_selected) {
				...
				case 5: /*FLOWER*/
					var numpoints=360; //number of points of flower
					drawAxis();	// instance to drawAxis	function
					processing.noFill(); // no fill shapes
					processing.stroke(0,0,255);   // stroke color RGB - blue
					processing.pushMatrix();	
					processing.translate (sizescreenx/2,sizescreeny/2);  //translate coordinates to the middle of screen
					if (v_npetalsflower>2) {radius1=180;} else {radius1=100;}
					radius2=radius1/v_npetalsflower;	
					pointshape=flower(radius1,radius2,numpoints);  //draw the flower and define pointshape
					processing.popMatrix(); 
				...
				case 6: /*ROUNDED STAR*/
					var numpoints=360; //number of points of rounded star
					drawAxis();	// instance to drawAxis	function
					processing.noFill(); // no fill shapes
					processing.stroke(0,0,255);   // stroke color RGB - blue
					processing.pushMatrix();	
					...
					radius1=300;
					radius2=radius1/v_ncurvesroundedstar;	
					pointshape=roundedstar(radius1,radius2,numpoints);  //draw the rounded star and define pointshape
					processing.popMatrix(); 
				...
				case 7: /*HEART*/
					var numpoints=360; //number of points of heart
					drawAxis();	// instance to drawAxis	function
					processing.noFill(); // no fill shapes
					processing.stroke(0,0,255);   // stroke color RGB - blue
					processing.pushMatrix();	
